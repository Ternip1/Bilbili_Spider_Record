...
file_4 = "最高评分rank.csv"
file_5 = "开播时间rank.csv"
url = "https://api.bilibili.com/pgc/season/index/result?season_version=-1&spoken_language_type=-1&area=-1&is_finish=-1&copyright=-1&season_status=-1&season_month=-1&year=-1&style_id=-1&order="+str(j)+"&st=1&sort=0&page="+str(i)+"&season_type=1&pagesize=20&type=1"
#追番人数rank
...
for i in range(1,163):
    res = requests.get(url,headers=headers)
    df = pd.DataFrame(res.json()['data']['list'])
    df.to_csv(file_0,index=False, mode='a',header=not os.path.exists(file_0), encoding="u8")
    i +=1
    time.sleep(random.randint(1,8))
    print("番剧更新rank:", i)
#爬取番剧播放人数rank
for i in range(1,163):
    res = requests.get(url,headers=headers)
    df = pd.DataFrame(res.json()['data']['list'])
    df.to_csv(file_2,index=False, mode='a',header=not os.path.exists(file_2), encoding="u8")
    i +=1
    time.sleep(random.randint(1,8))
    print("播放人数rank:", i)
#最高评分rank
for i in range(1,163):
    res = requests.get(url,headers=headers)
    df = pd.DataFrame(res.json()['data']['list'])
    df.to_csv(file_4,index=False, mode='a',header=not os.path.exists(file_4), encoding="u8")
    i +=1
    time.sleep(random.randint(1,8))
    print("最高评分rank:", i)
#开播时间ank
for i in range(1,163):
    res = requests.get(url,headers=headers)
    df = pd.DataFrame(res.json()['data']['list'])
    df.to_csv(file_5,index=False, mode='a',header=not os.path.exists(file_5), encoding="u8")
    i +=1
    time.sleep(random.randint(1,8))
    print("开播时间rank:",i)
